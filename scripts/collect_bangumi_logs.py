...
USER_AGENT = "Mozilla/5.0 (compatible; bangumi-comment-skill/0.2; +https://bangumi.tv/)"
...
def collect(domain: str, target: int, max_pages: int, min_chars: int, min_paragraphs: int) -> tuple[list[BlogSample], dict]:
    ...
    for entry in iter_list_entries(domain, max_pages=max_pages):
        ...
        try:
            ...
            print(f"WARN failed {entry.url}: {exc}", file=sys.stderr)
            continue
        ...
        if scanned % 25 == 0:
            print(
                f"progress domain={domain} scanned={scanned} accepted={len(accepted)} rejected={rejected} last_page={last_page}",
                file=sys.stderr,
            )
